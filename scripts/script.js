document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchPosts() {
        ...
        try {
            ...
            posts.sort((a, b) => b.status - a.status);//Deve estar listado como as favoritas em primeiro lugar seguido de outros posts
            
            listaPosts.innerHTML = '';//Limpa a mensagem padrão
            posts.forEach(post => {
                const div = document.createElement('div');
                div.className = 'post p-3 rounded shadow-sm bg-dark';
                div.innerHTML = `
                <div class="post-header d-flex justify-content-between align-items-center mb-2">
                    <small class="text-white-50">${dataFormatoHtml(post.data_criacao)}</small>
                    <div>
                        <div id="post_lista">
    ...
    async function handleFormSubmit(event) {
        ...
        const data_criacao = dataFormatoSql(new Date());
    ...
    function dataFormatoHtml(dateString) {
